...
create table if not exists movies_metadataproduction_companies(id int,name varchar(150),autoId int not null primary key,movies_metadataid int not null, foreign key fk_movies_metadata(movies_metadataid) references movies_metadata(id))engine=InnoDB default charset=utf8;
create table if not exists movies_metadataproduction_countries(iso_3166_1 varchar(10),name varchar(100),autoId int not null primary key,movies_metadataid int not null, foreign key fk_movies_metadata(movies_metadataid) references movies_metadata(id))engine=InnoDB default charset=utf8;
